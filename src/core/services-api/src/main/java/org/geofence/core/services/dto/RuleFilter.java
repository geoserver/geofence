...
public class RuleFilter implements Cloneable {
    ...
    public enum SpecialFilterType {
        ...
        /**
         * ANY will not add any costraints on the field. This means that all values on that field will be allowed. Pay
         * attention when using this type, since it may allow rules you do not intended to retrieve. You may mean to use
         * the ANY value.
         */
        ANY(FilterType.ANY),
        /**
         * DEFAULT will create an IS NULL filter on the field. This means only the default rules for a given field will
         * be returned. This is probably the value you want to use.
         */
    ...
    private String sortNames(String s) {
        if (s.contains(",")) {
            s = Arrays.asList(s.split(",")).stream().map(n -> n.trim()).sorted().collect(Collectors.joining(","));
        }
        return s;
    }
    ...
    public static class IdNameFilter implements Serializable, Cloneable {
        ...
        /**
         * Only used in TYPE_ID and TYPE_NAME, tells if also default Rules should be matched. Old code automatically
