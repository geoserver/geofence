...
public class RuleReaderServiceImplTest extends ServiceTestBase {
    ...
    public void testGetRulesForUsersAndGroup() {
        ...
        assertEquals(
                3, getMatchingRules("*", "p1", "Z", "*", "*", "*", "*", "*").size());
        assertEquals(
                1, getMatchingRules(u1, "*", "Z", "*", null, null, null, null).size());
        assertEquals(
                0, getMatchingRules("*", "Z", "Z", "*", null, null, null, null).size());
        assertEquals(
                1, getMatchingRules(u1, "*", "Z", "*", null, null, null, null).size());
        assertEquals(
                1, getMatchingRules(u1, "*", "Z", "*", null, null, null, null).size());
        ...
        assertEquals(
                1, getMatchingRules("*", "p2", "Z", "*", "*", "*", "*", "*").size());
        assertEquals(2, getMatchingRules(u1, "*", "Z", "*", "s1", "*", "*", "*").size());
        assertEquals(
                2, getMatchingRules("*", "p1", "Z", "*", "s1", "*", "*", "*").size());
    ...
    public void testGetRulesForGroupOnly() {
        ...
        assertEquals(
                3, getMatchingRules("*", "*", "*", "*", "s1", "*", "*", "*").size());
        assertEquals(
                1, getMatchingRules("*", "*", "*", "*", "ZZ", "*", "*", "*").size());

        assertEquals(
                3, getMatchingRules("*", "p1", "*", "*", "*", "*", "*", "*").size());
        assertEquals(
                2, getMatchingRules("*", "p1", "*", "*", "s1", "*", "*", "*").size());
        assertEquals(
                1, getMatchingRules("*", "p1", "*", "*", "ZZ", "*", "*", "*").size());

        assertEquals(
                1, getMatchingRules("*", "p2", "*", "*", "*", "*", "*", "*").size());
        assertEquals(
                1, getMatchingRules("*", "p2", "*", "*", "s1", "*", "*", "*").size());
        assertEquals(
                0, getMatchingRules("*", "p2", "*", "*", "ZZ", "*", "*", "*").size());
    ...
    public void testGetInfo() {
        ...
        RuleFilter baseFilter = new RuleFilter(RuleFilter.SpecialFilterType.ANY)
                ...
                .setInstance("i0")
                .setService("WCS")
                .setRequest(RuleFilter.SpecialFilterType.ANY)
                .setWorkspace("W0")
                .setLayer("l0");

        {
            RuleFilter ruleFilter = new RuleFilter(baseFilter).setUser(SpecialFilterType.ANY);
        ...
        }
        {
            RuleFilter ruleFilter =
                    new RuleFilter(baseFilter).setUser(SpecialFilterType.ANY).setService("UNMATCH");
        ...
        {
            RuleFilter ruleFilter =
                    new RuleFilter(baseFilter).setRole(SpecialFilterType.ANY).setService("UNMATCH");
    ...
    public void testResolveLazy() {
        ...
        rules.get(1).setLayerDetails(details);
    ...
    public void testGroupOrder01() throws UnknownHostException {
        ...
        RuleFilter filterU1 = new RuleFilter(RuleFilter.SpecialFilterType.ANY).setUser(u1.getName());

        RuleFilter filterU2 = new RuleFilter(RuleFilter.SpecialFilterType.ANY).setUser(u2.getName());
    ...
    public void testAttrib() throws NotFoundServiceEx {
        ...
        // TEST u1
        {
            RuleFilter filterU1 = new RuleFilter(RuleFilter.SpecialFilterType.ANY).setUser("u1");
    ...
    public void testNullAllowableStyles() throws NotFoundServiceEx {
...

        {
            ...
            // no details for first rule
            {
                ...
                ruleAdminService.insert(r1);
                Rule reloaded = ruleAdminService.get(r1.getId());
                assertNotNull(reloaded, "Rule " + r1.getId() + " not found");
            }
    ...
    public void testIPAddress() {
        ...
        assertEquals(4, getMatchingRules("*", "*", "*", "*", "*", "*", "*", "*").size());
        assertEquals(
                3, getMatchingRules("*", "g1", "*", "*", "*", "*", "*", "*").size());
        assertEquals(
                1, getMatchingRules("*", "g2", "*", "*", "*", "*", "*", "*").size());
        assertEquals(
                2, getMatchingRules("*", "g1", "*", "*", "s1", "*", "*", "*").size());
        assertEquals(
                1, getMatchingRules("*", "*", "*", "*", "ZZ", "*", "*", "*").size());
        ...
        assertEquals(
                1,
                getMatchingRules("*", "*", "*", "10.10.1.4", "*", "*", "*", "*").size());
        assertEquals(
                2,
                getMatchingRules("*", "*", "*", "192.168.1.1", "*", "*", "*", "*")
                        .size());
        assertEquals(
                1, getMatchingRules("*", "*", "*", null, "*", "*", "*", "*").size());

        assertEquals(
                0, getMatchingRules("*", "*", "*", "BAD", "*", "*", "*", "*").size());
    }
    ...
    public void testAdminRules() {
        ...
        ruleAdminService.insert(
                new Rule(10, user.getName(), null, null, null, "s1", "r1", null, "w1", "l1", GrantType.ALLOW));
    ...
    public void testDateRange() throws Exception {
        ...
        ruleAdminService.insert(new Rule(10, GrantType.ALLOW));
        ruleAdminService.insert(new Rule(20, GrantType.ALLOW).setValidBefore(Date.valueOf("2025-01-10")));
        ruleAdminService.insert(new Rule(30, GrantType.ALLOW).setValidAfter(Date.valueOf("2025-02-10")));
        ruleAdminService.insert(new Rule(40, GrantType.ALLOW)
                .setValidAfter(Date.valueOf("2025-03-10"))
                .setValidBefore(Date.valueOf("2025-04-10")));

        assertRules(getDateFilter(null, true), new Integer[] {10, 20, 30, 40});
        assertRules(getDateFilter("2025-01-05", true), new Integer[] {10, 20});
        assertRules(getDateFilter("2025-01-05", false), new Integer[] {20});
        assertRules(getDateFilter("2025-01-15", true), new Integer[] {10});
        assertRules(getDateFilter("2025-01-15", false), new Integer[] {});
        assertRules(getDateFilter("2025-02-15", true), new Integer[] {10, 30});
        assertRules(getDateFilter("2025-03-15", true), new Integer[] {10, 30, 40});
        assertRules(getDateFilter("2025-04-15", true), new Integer[] {10, 30});
    }

    private static RuleFilter getDateFilter(String date, boolean includeDefault) {
        RuleFilter r = new RuleFilter(RuleFilter.SpecialFilterType.ANY);
        if (date != null) r.setDate(date);
        r.getDate().setIncludeDefault(includeDefault);
        return r;
    }
     ...
     * </UL>
     */
    private RuleFilter createHeuristicFilter(
            String userName,
            String groupName,
            String instanceName,
    ...
    private List<ShortRule> getMatchingRules(
        ...
        return ruleReaderService.getMatchingRules(createHeuristicFilter(
                userName, profileName, instanceName, sourceAddress, service, request, null, workspace, layer));
    }
    ...
    private AccessInfo getAccessInfo(
            ...
            String workspace,
            String layer) {
        return ruleReaderService.getAccessInfo(createHeuristicFilter(
                userName, roleName, instanceName, sourceAddress, service, request, null, workspace, layer));
    }
}
