...
public class RuleAdminServiceImplTest extends ServiceTestBase {
    ...
    @Test
    public void testInsertDeleteRule(TestInfo testInfo) throws NotFoundServiceEx {
        String methodName = testInfo.getTestMethod().get().getName();

        UserGroup profile = createRole(methodName);
        Rule rule = new Rule();
        rule.setRolename(methodName);
        rule.setAccess(GrantType.ALLOW);
        ruleAdminService.insert(rule);
        ruleAdminService.get(rule.getId()); // will throw if not found
        assertTrue(ruleAdminService.delete(rule.getId()), "Could not delete rule");
    }
    ...
    public void testAttribs() throws NotFoundServiceEx {
        ...
        final Long lid1;
        {
            ...
            details.setAttributes(new HashSet<LayerAttribute>(Arrays.asList(
                    new LayerAttribute("attr1", AccessType.NONE), new LayerAttribute("attr2", AccessType.READWRITE))));
            ruleAdminService.setDetails(id, details);
            lid1 = details.getId();
            assertNotNull(lid1);
        }
        ...
        // check attribs have been properly set
        {
            ...
            assertEquals(
                    new HashSet<LayerAttribute>(Arrays.asList(
