...
import org.geofence.core.db.dao.SearchableDAO;
...
public class FilterUtils {
    ...
    public static <RULETYPE extends IPRangeProvider> List<RULETYPE> getFilteredRules(
            SearchableDAO<RULETYPE> dao, RuleFilter filter) {
        return getFilteredRules(dao, filter, null, null);
    }

    public static <RULETYPE extends IPRangeProvider> List<RULETYPE> getFilteredRules(
            SearchableDAO<RULETYPE> dao, RuleFilter filter, Integer page, Integer entries) {
    ...
    public static long countFilteredRules(SearchableDAO dao, RuleFilter filter) {
    ...
    public static <T> List<T> paginateList(List<T> list, Integer page, Integer entries) {
     ...
     *
     * <p>IP address filtering is not performed by DAO at the moment, so we'll have to filter out such results by hand.
     */
     ...
     *
     * <p>We're dealing with IDs here, so <U>we'll suppose that the related object id field is called "id"</U>.
     */
        ...
        switch (filter.getType()) {
            ...
            case IDVALUE:
                if (filter.isIncludeDefault()) {
                    search.addFilterOr(search.isNull(join.getField()), search.isEqual(join, "id", filter.getId()));
                } else {
            ...
            case NAMEVALUE:
                if (filter.isIncludeDefault()) {
                    search.addFilterOr(search.isNull(join.getField()), search.isEqual(join, "name", filter.getName()));
                } else {
    ...
    public static void addStringCriteria(BaseSearch search, String fieldName, RuleFilter.TextFilter filter) {
        switch (filter.getType()) {
            ...
            case NAMEVALUE:
                if (filter.isIncludeDefault()) {
                    search.addFilterOr(search.isNull(fieldName), search.isEqual(fieldName, filter.getText()));
                } else {
    ...
    public static void addDateCriteria(BaseSearch search, RuleFilter.TextFilter filter) {
        ...
        switch (filter.getType()) {
            ...
            case NAMEVALUE:
                ...
                search.addFilterOr(search.isNull(AFTER), search.isAfter(AFTER, date));
                search.addFilterOr(search.isNull(BEFORE), search.isBefore(BEFORE, date));

                if (!filter.isIncludeDefault()) {
                    // we need at least one field not null to tell whether the date filtering is active
                    search.addFilterOr(search.isNotNull(AFTER), search.isNotNull(BEFORE));
                }
                break;
