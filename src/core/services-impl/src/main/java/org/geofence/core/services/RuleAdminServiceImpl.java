 ...
 * not case sensitive, so we're going to turn all of them uppercase. See also {@link RuleReaderServiceImpl}.
 *
...
public class RuleAdminServiceImpl implements RuleAdminService {
    ...
    /**
     * <TT>service</TT> and <TT>request</TT> params are usually set by the client, and by OGC specs they are not case
     * sensitive, so we're going to turn all of them uppercase. See also {@link RuleReaderServiceImpl}.
     */
    ...
    public void deleteRulesByUser(String username) throws NotFoundServiceEx {
        ...
        if (LOGGER.isInfoEnabled()) LOGGER.info("Removing " + list.size() + " rules for user " + username);
        for (Rule rule : list) {
    ...
    public void deleteRulesByInstance(long instanceId) throws NotFoundServiceEx {
        ...
        List<Rule> list = ruleDAO.search(searchCriteria);
        for (Rule rule : list) {
            if (LOGGER.isInfoEnabled()) LOGGER.info("Removing rule for instance " + instanceId + ": " + rule);
            ruleDAO.remove(rule);
        }
    }
    ...
    public void setDetails(Long ruleId, LayerDetails detailsNew) {
        ...
        LayerDetails oldDetails = rule.getLayerDetails();
        if (oldDetails != null) {
            oldStyles = detailsDAO.getAllowedStyles(ruleId);
            oldAttrs = oldDetails.getAttributes();
        ...
        if (rule.getLayerDetails() != null) throw new IllegalStateException("LayerDetails should be null");
        ...
        if (detailsNew != null) {
            ...
            if (detailsNew.getAttributes() == null) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    if (oldAttrs != null && oldAttrs.equals(detailsNew.getAttributes())) {
                        ...
                        LOGGER.debug("Setting " + detailsNew.getAttributes().size() + " new attrs "
            ...
            rule.setLayerDetails(detailsNew);
