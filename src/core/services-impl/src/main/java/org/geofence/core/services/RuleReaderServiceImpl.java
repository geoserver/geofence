...
import org.geofence.core.db.dao.RuleDAO;
...
/**
 * <B>Note:</B> <TT>service</TT> and <TT>request</TT> params are usually set by the client, and by OGC specs they are
 * not case sensitive, so we're going to turn all of them uppercase. See also {@link RuleAdminServiceImpl}.
 *
...
public class RuleReaderServiceImpl implements RuleReaderService {
    ...
    public List<ShortRule> getMatchingRules(RuleFilter filter) {
        ...
        LOGGER.info(sorted.size() + " matching rules for filter " + filter);
    ...
    private void setAllowedAreas(AccessInfoInternal baseAccess, AccessInfoInternal moreAccess, AccessInfoInternal ret) {
        ...
        if (unionIntersects == null) {
            if (baseIntersects != null && moreClip != null) ret.setArea(baseIntersects);
            else if (moreIntersects != null && baseClip != null) ret.setArea(moreIntersects);
        } else {
            ret.setArea(unionIntersects);
        }
        if (unionClip == null) {
            if (baseClip != null && moreIntersects != null) ret.setClipArea(baseClip);
            else if (moreClip != null && baseIntersects != null) ret.setClipArea(moreClip);
        } else {
    ...
    private AccessInfoInternal buildAllowAccessInfo(Rule rule, List<RuleLimits> limits, IdNameFilter userFilter) {
        ...
        boolean atLeastOneClip =
                limits.stream().anyMatch(l -> l.getSpatialFilterType().equals(SpatialFilterType.CLIP));
        ...
        if (area != null) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                        "Attaching an area to Accessinfo: " + area.getClass().getName() + " " + area.toString());
            }
            // if we have a clip area we apply clip type
            // since is more restrictive, otherwise we keep
            // the intersect
            if (atLeastOneClip) accessInfo.setClipArea(area);
            else accessInfo.setArea(area);
        }
        return accessInfo;
    }
    ...
    protected static CatalogMode getLarger(CatalogMode m1, CatalogMode m2) {
        ...
        if (CatalogMode.CHALLENGE == m1 || CatalogMode.CHALLENGE == m2) return CatalogMode.CHALLENGE;
     ...
     *
     * <p>Compatible filters:
     * <li>username assigned and rolename:ANY -> should consider all the roles the user belongs to
     * <li>username:ANY and rolename assigned -> should consider all the users belonging to the given role
     *
     * @param filter a RuleFilter for rule selection. <B>side effect</B> May be changed by the method
     * @return a Map having role names as keys, and the list of matching Rules as values. The NULL key holds the rules
    ...
    protected Map<String, List<Rule>> getRules(RuleFilter filter) throws BadRequestServiceEx {
        ...
        if (finalRoleFilter.isEmpty()) {
            TextFilter roleFilter = new TextFilter(filter.getRole().getType())
            ...
            for (String role : finalRoleFilter) {
                TextFilter roleFilter = new TextFilter(role).setIncludeDefault(true);
                ...
                if (username != null) {
                    ...
                    for (String role : requestedRoles) {
                        if (resolvedRoles.contains(role)) {
                            ...
                            LOGGER.warn("User does not belong to role [User:" + filter.getUser() + "] [Role:" + role
