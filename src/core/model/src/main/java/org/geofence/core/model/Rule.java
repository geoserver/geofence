 ...
 *   <LI>the group associated to the requesting user
 *   <LI>the instance of the accessed geoserver
 *   <LI>the ip address of the requesting client
 *   <LI>the date the request is performed
 ...
 * </UL>
 *
 * <p><B>Example</B>: In order to allow access to every request to the WMS service in the instance GS1, you will need to
 * create a Rule, by only setting Service=WMS and Instance=GS1, leaving the other fields to <TT>null</TT>.
 *
 * <p>When an access has to be checked for filtering, all the matching rules are read; they are then evaluated according
 * to their priority: the first rule found having accessType <TT><B>{@link GrantType#ALLOW}</B></TT> or
 * <TT><B>{@link GrantType#DENY}</B></TT> wins, and the access is granted or denied accordingly. <br>
 * Matching rules with accessType=<TT><B>{@link GrantType#LIMIT}</B></TT> are collected and evaluated at the end, only
 * if the request is Allowed by some other rule with lower priority. <br>
 * These rules will have an associated {@link RuleLimits RuleLimits} that defines some restrictions for using the data
        ...
        uniqueConstraints = { // @InternalModel
            @UniqueConstraint(
                    columnNames = { // @InternalModel
                        "username",
                        "rolename",
                        "instance_id", // @InternalModel
                        "ip_low",
                        "ip_high",
                        "ip_size",
                        "valid_after",
                        "valid_before", // @InternalModel
                        "service",
                        "request",
                        "subfield",
                        "workspace",
...
public class Rule implements Identifiable, Serializable, Prioritizable, IPRangeProvider {
    ...
    @OneToOne(optional = true, cascade = CascadeType.ALL, orphanRemoval = true, mappedBy = "rule")
    ...
    @OneToOne(
            ...
            orphanRemoval = true,
            mappedBy = "rule") // main ref is in ruleLimits
