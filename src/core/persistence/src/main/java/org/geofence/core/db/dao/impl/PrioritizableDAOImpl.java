...
import org.geofence.core.db.dao.PrioritizableDAO;
import org.geofence.core.db.dao.search.BaseSearch.Field;
...
import org.geofence.core.model.Prioritizable;
...
public abstract class PrioritizableDAOImpl<T extends Prioritizable & Identifiable> extends BaseDAOImpl<T, Long>
            ...
            case FROM_START:
                ...
                if (list.isEmpty()) { // no rule found at given position: let's find out why
                    ...
                    if (count == 0) { // this is the first rule inserted
                        ...
                        if (LOGGER.isDebugEnabled()) LOGGER.debug("Inserting rule in last position");
                    }
                } else {
    ...
    /**
     * This is needed to call the proper persist() metod in the child class, or automatic bean conversion in DAO will
     * not be carried out.
     */
    ...
    protected int shift(Class<T> clazz, long priorityStart, long offset) {
        ...
        String hql =
                "UPDATE " + clazz.getSimpleName() + " SET priority=priority+ :offset WHERE priority >= :priorityStart";
