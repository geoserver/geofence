...
import org.geofence.core.db.dao.search.SearchUtil;
...
public class RuleDAOTest extends BaseDAOTest {
    ...
    public void testPersistRule() throws Exception {
        ...
        // test save & load
        {
            ...
            assertNotNull(loaded, "Can't retrieve rule");
    ...
    public void setDetails(Long ruleId, LayerDetails details) {
        ...
        if (rule.getLayerDetails() != null) throw new IllegalStateException("LayerDetails (1) should be null");

        if (detailsDAO.find(ruleId) != null) throw new IllegalStateException("LayerDetails (2) should be null");

        if (!detailsDAO.findAll().isEmpty()) {
            LOGGER.error("LayerDetails (3) should be null --> " + detailsDAO.findAll());
            throw new IllegalStateException("LayerDetails (3) should be null --> "
                    + detailsDAO.findAll().size());
        }
        ...
        if (details != null) {
            ...
            //                LOGGER.info("Restoring " + oldProps.size() + " props from older LayerDetails
            // (id:"+ruleId+")");
    ...
    public void testShift() {
        assertEquals(0, ruleDAO.count(null));
        ...
        Search<Rule> s = ruleDAO.createSearch();
    ...
    public void testPersistRulePosition() throws Exception {
        ...
        {
            assertEquals(0, ruleDAO.count(null));
            Rule rule1 = new Rule(1000, GrantType.ALLOW).setService("s");
        ...
        ruleDAO.persist(new Rule(10, GrantType.ALLOW).setService("s10"));
        ruleDAO.persist(new Rule(20, GrantType.ALLOW).setService("s20"));

        {
            assertEquals(3, ruleDAO.count(null));
            Rule rule1 = new Rule(1000, GrantType.ALLOW).setService("sZ");
            long pri = ruleDAO.persist(rule1, InsertPosition.FROM_START);
            assertEquals(21, pri);
        }

        {
            Rule rule1 = new Rule(1, GrantType.ALLOW).setService("second");
            long pri = ruleDAO.persist(rule1, InsertPosition.FROM_START);
            assertEquals(10, pri);
        }

        {
            Rule rule1 = new Rule(0, GrantType.ALLOW).setService("last");
            long pri = ruleDAO.persist(rule1, InsertPosition.FROM_END);
            assertEquals(23, pri);
        }

        {
            Rule rule1 = new Rule(1, GrantType.ALLOW).setService("last2");
            long pri = ruleDAO.persist(rule1, InsertPosition.FROM_END);
            assertEquals(23, pri);
        }
    }
    ...
    public void testIPRangeTest() throws Exception {
        ...
        Long rid;
        {
            ...
            Rule rule = new Rule(0, GrantType.ALLOW).setUsername("rule_test").setAddressRange("10.11.0.0/16");
            ruleDAO.persist(rule);
        ...
        // test update
        {
            ...
            // DO REMOVE
            loaded.setAddressRange((IPAddressRange) null);
            ruleDAO.merge(loaded);
        }
