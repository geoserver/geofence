 ...
 *
 * <p>Parses the input params and performs some validations on them.
 */
public class Cvs2Xml {

    private static final Logger LOGGER = LogManager.getLogger(Cvs2Xml.class);
    ...
    protected static RunInfo parse(String[] args) {
        ...
        } else LOGGER.info("No user definition file");
        ...
        } else LOGGER.info("No rule definition file");
        ...
        if (cli.hasOption(CLI_DELETERULES_LONG)) runInfo.setDeleteObsoleteRules(true);
    ...
    protected static boolean validate(RunInfo runInfo) {
        ...
        if (runInfo.getOutputFile() != null) {
            if (!runInfo.getOutputFile().getAbsoluteFile().getParentFile().canWrite()) {
                LOGGER.error("Can't write xml command file "
                        + runInfo.getOutputFile().getAbsolutePath());
                return false;
            }
        }
    ...
    protected static Options createCLIOptions() throws IllegalArgumentException {
        ...
        options.addOption(OptionBuilder.withDescription("Send commands to GeoFence")
                .withLongOpt("send")
        ...
        options.addOption(OptionBuilder.withDescription("Delete obsolete rules")
