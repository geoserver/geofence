...
public interface RESTGSInstanceService {
    ...
    @Path("/id/{id}")
    @Produces(MediaType.APPLICATION_XML)
    RESTOutputInstance get(@PathParam("id") Long id)
            throws BadRequestRestEx, NotFoundRestEx, InternalErrorRestEx;

    @GET
    @Path("/name/{name")
    @Produces(MediaType.APPLICATION_XML)
    RESTOutputInstance get(@PathParam("name") String name)
            throws BadRequestRestEx, NotFoundRestEx, InternalErrorRestEx;

    @POST
    @Path("/")
    @Produces(MediaType.TEXT_PLAIN)
    Response insert(@Multipart("instance") RESTInputInstance instance)
            throws BadRequestRestEx, ConflictRestEx, InternalErrorRestEx;

    @PUT
    @Path("/id/{id}")
    @Produces(MediaType.APPLICATION_XML)
    void update(@PathParam("id") Long id, @Multipart("instance") RESTInputInstance instance)
            throws BadRequestRestEx, NotFoundRestEx, InternalErrorRestEx;

    @PUT
    @Path("/name/{name}")
    @Produces(MediaType.APPLICATION_XML)
    void update(@PathParam("name") String name, @Multipart("instance") RESTInputInstance instance)
            throws BadRequestRestEx, NotFoundRestEx, InternalErrorRestEx;
     ...
     * @throws ConflictRestEx (HTTP code 409) if any rule refers to the instance and cascade is
     *     false
    ...
    @Path("/id/{id}")
    Response delete(
            @PathParam("id") Long id, @QueryParam("cascade") @DefaultValue("false") boolean cascade)
