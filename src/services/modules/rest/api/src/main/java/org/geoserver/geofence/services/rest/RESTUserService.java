...
public interface RESTUserService {
     ...
     * @param page In a paginated list, the page number, 0-based. If not null, <code>entries</code>
     *     must be defined as well.
     * @param entries In a paginated list, the number of entries per page. If not null, <code>page
     *     </code> must be defined as well.
    ...
    Response insert(@Multipart("user") RESTInputUser user)
    ...
    @PUT
    @Path("/id/{id}")
    void update(@PathParam("id") Long id, @Multipart("user") RESTInputUser user)
    ...
    @PUT
    @Path("/name/{name}")
    void update(@PathParam("name") String name, @Multipart("user") RESTInputUser user)
            throws BadRequestRestEx, NotFoundRestEx;
